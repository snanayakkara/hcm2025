...
const ReferralForm: React.FC<ReferralFormProps> = ({ isOpen, onClose }) => {
  ...
  // Ref for the scrollable form wrapper
  const scrollWrapperRef = useRef<HTMLDivElement | null>(null);

  // Ensure horizontal scroll position is always at the far left on open / resize
  useEffect(() => {
    if (isOpen && scrollWrapperRef.current) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <div 
            className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto"
            style={{ width: '1200px', maxWidth: '95vw' }}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="sticky top-0 bg-white border-b border-gray-200 px-3 sm:px-4 lg:px-6 py-3 sm:py-4 rounded-t-2xl z-10">
            ...
            >
              ...
              <section>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 px-4 w-full min-w-0">
                  {referralTypes.map((type) => (
                    <button
                      key={type.value}
                      type="button"
                      onClick={() => handleReferralTypeToggle(type.value)}
                      className={`relative overflow-hidden w-full px-2 py-3 text-xs font-medium rounded-lg border-2 transition-all duration-300 min-h-[70px] ${
              ...
              <section className="relative w-full max-w-full min-w-0">
                <div className="flex items-center space-x-2 mb-4">
                  <User className="w-5 h-5 text-blue-600" />
                  <h3 className="text-lg font-semibold text-gray-900">Patient Details</h3>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 w-full max-w-full min-w-0">
                  <div>
              ...
              <section className="relative w-full max-w-full min-w-0">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-3 w-full max-w-full min-w-0 relative">
